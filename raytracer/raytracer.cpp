...
#include "tracers/improvedTracer.hpp"
int main(int argc, char **argv) {
  ...
  Tracer *tracer = new improvedTracer(world);
  std::vector<Ray> rays;
  for (int x = 0; x < viewplane.hres; x++) {   // across.
    for (int y = 0; y < viewplane.vres; y++) { // down.
      ...
      for (const auto &ray : rays) {
        ...
        ShadeInfo sinfo = tracer->hit_objects(ray);
        if (sinfo.hit) {

          pixel_color += weight * tracer->shade(sinfo); //sinfo.material_ptr->shade(sinfo);
        }
	else {
